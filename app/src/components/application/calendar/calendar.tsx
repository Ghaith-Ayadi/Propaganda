...
const viewOptions: ViewOption[] = [{ value: "month", label: "Month view", addon: "⌘M" }];
...
const getDaysInVisibleMonthGrid = (anchorDate: CalendarDate, locale: string): CalendarDate[] => {
    ...
    const weekStart = startOfWeek(monthStart, locale, "mon");
    const weekEnd = endOfWeek(monthEnd, locale, "mon");
...
const MonthView = ({
    ...
    const weekdays = useMemo(() => {
        const firstDay = startOfWeek(today(timeZone), locale, "mon");
                        ...
                        >
                            <div className="flex gap-1 max-md:pl-1 md:flex-col">
                                {eventsToShow.map(({ event, span, isPlaceholder }) => {
                                    ...
                                    return (
                                        <div key={event.id} data-event-id={event.id} style={spanStyle} className={cx(!isCurrentMonthFlag && "opacity-50")}>
