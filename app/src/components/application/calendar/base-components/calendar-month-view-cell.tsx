...
export const CalendarMonthViewCell = ({ isDisabled, children, className, day, state, dateKey, ...props }: CalendarMonthViewCellProps) => {
    return (
        <div
            {...props}
            className={cx(
                ...
                isDisabled ? "pointer-events-none bg-secondary_alt" : "cursor-pointer",
                className,
            )}
        >
            {!isDisabled && (
                <div data-add-date={dateKey} className="absolute right-1.5 bottom-1.5 z-10 hidden group-hover:inline-flex">
                    <Button aria-label="Add brief" size="sm" iconLeading={Plus} color="secondary" className="size-7 text-fg-quaternary" />
                </div>
            )}
