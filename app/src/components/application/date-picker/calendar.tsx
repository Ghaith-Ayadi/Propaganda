...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // react-aria-components 1.18 widened the calendar context onChange type to
    // single | range; this is a single-date calendar, so the cast is safe.
    return (
        <AriaCalendarContext.Provider value={{ value, onChange, focusedValue, onFocusChange } as never}>
            {children}
